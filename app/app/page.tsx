        ...
        <div className="lg:col-span-5 space-y-6">
            ...
            <CardContent className="pt-6">
              {isAddingProfile ? (
                <div className="space-y-3 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <input placeholder="岗位名称 (例如: 高级数据分析师)" className="w-full p-2.5 bg-zinc-50 border border-zinc-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" value={newProfileName} onChange={(e) => setNewProfileName(e.target.value)} />
                  ...
                  <Button className="w-full bg-blue-600 hover:bg-blue-700 h-11 shadow-sm" onClick={handleSaveProfile}>{editingProfileId ? "更新画像数据" : "保存至画像库"}</Button>
                </div>
              ) : (
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1">
                      <Select value={selectedProfileId} onValueChange={setSelectedProfileId}>
                        <SelectTrigger className="h-11 border-zinc-200 w-full bg-white shadow-sm"><SelectValue placeholder="选择目标岗位画像" /></SelectTrigger>
          ...
          <Card className="border-none shadow-sm ring-1 ring-zinc-200">
            ...
            <CardContent className="space-y-4">
              <Textarea placeholder="在此粘贴简历文本..." className="h-[400px] bg-zinc-50/50 resize-none text-sm border-zinc-200 focus:ring-2 focus:ring-blue-500/20" value={resume} onChange={(e) => setResume(e.target.value)} />
          ...
          <div className="space-y-6 lg:h-[835px] overflow-y-auto pr-3 pb-10 custom-scrollbar scroll-smooth">
            {jobs.length === 0 && !loading ? (
              ...
              jobs.map((job) => {
                ...
                return (
                  // ⭐ 核心修改：优化卡片样式 (阴影、边框、悬停动效)
                  <Card 
                    key={job.id} 
                    className="border border-zinc-200/60 shadow-sm bg-white hover:shadow-md hover:-translate-y-[2px] transition-all duration-300 overflow-hidden animate-in fade-in slide-in-from-right-4"
                  >
                    <CardContent className="pt-6">
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        ...
                        <div className="border-t border-zinc-100 pt-2">
                          <button onClick={() => toggleExpand(job.id)} className="flex items-center gap-1 text-[10px] text-zinc-400 hover:text-blue-600 py-2 transition-colors">
                            {isExpanded ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                            {isExpanded ? "隐藏简历原文" : "查看原始简历备份"}
                          </button>
                          {isExpanded && <div className="mt-2 p-4 bg-zinc-50 rounded-xl text-[11px] text-zinc-500 font-mono whitespace-pre-wrap leading-relaxed max-h-60 overflow-y-auto border border-zinc-100 animate-in fade-in slide-in-from-top-1">{job.resume_text}</div>}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
