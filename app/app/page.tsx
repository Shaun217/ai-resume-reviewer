...
import { Loader2, Send, History, LogOut, CheckCircle2, ChevronDown, ChevronUp, PlusCircle, Target, FileText, Edit3, Sparkles, Cpu } from "lucide-react";
...
export default function AppPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb] selection:bg-blue-100 selection:text-blue-700">
      {/* 优化后的磨砂导航栏 */}
      <nav className="border-b bg-white/70 backdrop-blur-md p-4 flex justify-between items-center sticky top-0 z-40">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-blue-600 font-medium flex items-center gap-1">
              <Cpu className="w-3 h-3" /> POWERED BY GEMINI 2.5 FLASH
            </p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="hidden md:block text-right">
             <p className="text-xs font-bold text-zinc-700">{user?.email?.split('@')[0]}</p>
             <p className="text-[10px] text-zinc-400">APAC Digital Analytics</p>
          </div>
      ...
      <main className="container mx-auto p-4 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              {isAddingProfile ? (
                <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
                  <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest flex items-center gap-1">
                    <Sparkles className="w-3 h-3" /> {editingProfileId ? "正在重新校准画像" : "初始化新岗位画像"}
                  </p>
                  ...
                  <Textarea placeholder="输入该岗位的 JD 与核心技术栈要求..." className="h-32 text-sm bg-zinc-50 border-zinc-200 custom-scrollbar" value={newProfileReq} onChange={(e) => setNewProfileReq(e.target.value)} />
                  <Button className="w-full bg-blue-600 hover:bg-blue-700 h-11 shadow-sm font-bold" onClick={handleSaveProfile}>{editingProfileId ? "同步更新" : "存入画像库"}</Button>
                </div>
              ) : (
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1">
                      <Select value={selectedProfileId} onValueChange={setSelectedProfileId}>
                        <SelectTrigger className="h-11 border-zinc-200 w-full bg-white font-medium"><SelectValue placeholder="请选择目标对标画像" /></SelectTrigger>
                  ...
                  {selectedProfileId && (
                    <div className="p-4 bg-blue-50/40 border border-blue-100 rounded-xl text-[11px] text-blue-700 animate-in fade-in slide-in-from-top-1">
                      <div className="flex items-center gap-1.5 font-bold mb-1.5 uppercase tracking-wider opacity-80">
                        <FileText className="w-3 h-3" /> 画像核心对标要求
                      </div>
          ...
          <Card className="border border-zinc-200/60 shadow-sm bg-white">
            ...
            <CardContent className="pt-6 space-y-4">
              ...
              <Button className="w-full bg-zinc-900 hover:bg-zinc-800 h-12 text-lg font-black tracking-wide shadow-lg transition-all active:scale-[0.98]" onClick={handleSubmit} disabled={loading || !selectedProfileId}>
                {loading ? <><Loader2 className="mr-2 animate-spin" />ANALYZING...</> : <><Send className="mr-2 w-5 h-5" />EXECUTE AI SCAN</>}
              </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="lg:col-span-7 flex flex-col h-full">
          <div className="flex items-center justify-between mb-4 px-1">
            ...
            <span className="text-[10px] text-zinc-400 font-mono">{jobs.length} RECORDS</span>
          </div>
          ...
          <div className="space-y-6 lg:h-[835px] overflow-y-auto pr-3 pb-10 custom-scrollbar scroll-smooth">
            {jobs.length === 0 && !loading ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-20 text-zinc-300 border-2 border-dashed border-zinc-200 rounded-2xl bg-white/30 backdrop-blur-sm">
                ...
                <p className="text-sm font-medium">READY FOR NEW ANALYSIS</p>
                <p className="text-[10px] opacity-60 mt-1 uppercase">Paste a resume to start the engine</p>
              </div>
            ) : (
              jobs.map((job) => {
                    ...
                    <CardContent className="pt-6">
                      <div className="flex justify-between items-start mb-6">
                        <div>
                          ...
                          <p className="text-[10px] text-zinc-400 font-mono">
                             TIMESTAMP: {new Date(job.created_at).toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false })}
                          </p>
                        </div>
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        <div className={`p-5 rounded-2xl border transition-all duration-500 ${isMatch ? 'bg-green-50/40 border-green-100/50 text-green-900' : 'bg-red-50/40 border-red-100/50 text-red-900'}`}>
                          <p className="text-[9px] font-black uppercase tracking-widest mb-2 opacity-50">Conclusion</p>
                          ...
                          {(res.highlights || []).length > 0 && (
                            <div className={`pt-4 border-t ${isMatch ? 'border-green-200/50' : 'border-red-200/50'}`}>
                              <p className="text-[9px] font-black uppercase tracking-widest mb-3 opacity-50">{isMatch ? 'Key Matching Highlights' : 'Resume Key Points'}</p>
                        ...
                        <div className="p-5 bg-zinc-50 border border-zinc-100 rounded-2xl">
                          ...
                          <ul className="space-y-2.5 text-sm text-zinc-600 font-medium">
                            {(res.risks || ["No critical mismatch detected."]).map((r: string, i: number) => (
                        ...
                        <div className="border-t border-zinc-100 pt-2">
                          <button onClick={() => toggleExpand(job.id)} className="flex items-center gap-1.5 text-[10px] text-zinc-400 hover:text-blue-600 transition-colors py-2 font-bold uppercase">
                            ...
                            {isExpanded ? "Hide Source" : "Review Source Data"}
                          </button>
                          {isExpanded && <div className="mt-2 p-4 bg-zinc-100/50 rounded-xl text-[10px] text-zinc-500 font-mono whitespace-pre-wrap leading-relaxed max-h-60 overflow-y-auto border border-zinc-100 animate-in fade-in slide-in-from-top-1 custom-scrollbar">{job.resume_text}</div>}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
