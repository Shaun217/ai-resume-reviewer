...
import { 
  ...
  Mail, Phone, Copy, ShieldAlert, CheckCircle
} from "lucide-react";
...
export default function AppPage() {
  ...
  const handleManualSubmit = async () => {
    ...
    if (!profile || !resume.trim()) return toast.error("请选择画像并粘贴内容");
    setLoading(true);
    try {
      await analyzeResume({ resumeText: resume, position: profile.name, jobRequirements: profile.requirements, userId: user.id });
      setResume("");
      toast.success("分析已完成");
      await fetchJobs();
    } catch (e) {
  ...
  const handleSaveProfile = async () => {
    if (!newProfileName.trim() || !newProfileReq.trim()) return toast.error("请完整填写");
    if (editingProfileId) {
      ...
      <nav className="border-b bg-white/80 backdrop-blur-md p-4 flex justify-between items-center sticky top-0 z-40 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-blue-600 font-bold flex items-center gap-1 uppercase tracking-widest mt-1"><Cpu className="w-3 h-3" /> Gemini 2.5 Flash</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          {user && <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-full shadow-sm text-xs font-semibold text-slate-600"><User className="w-3.5 h-3.5 text-blue-600" /> {user.email}</div>}
          <Button variant="ghost" size="sm" className="hover:text-red-600" onClick={async () => { await supabase.auth.signOut(); window.location.href = "/"; }}><LogOut className="w-4 h-4 mr-2" /> 退出</Button>
        </div>
      </nav>
        ...
        <div className="lg:col-span-5 space-y-6">
          <Card className="border-slate-200 shadow-sm overflow-hidden bg-white">
            <CardHeader className="border-b border-slate-100 pb-4 bg-slate-50/50 flex flex-row justify-between items-center">
              ...
              <Button variant="outline" size="sm" onClick={() => { setIsAddingProfile(!isAddingProfile); setEditingProfileId(null); setNewProfileName(""); setNewProfileReq(""); }}>{isAddingProfile ? "取消" : <PlusCircle className="w-4 h-4 mr-1" />}</Button>
            </CardHeader>
            <CardContent className="pt-6">
              {isAddingProfile ? (
                <div className="space-y-4 animate-in fade-in duration-200">
                  ...
                  <Button className="w-full bg-blue-600 hover:bg-blue-700 h-11 font-bold" onClick={handleSaveProfile}>保存并同步画像</Button>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  <Select value={selectedProfileId} onValueChange={setSelectedProfileId}>
                    <SelectTrigger className="h-11 border-slate-200 w-full font-medium"><SelectValue placeholder="请选择对标画像" /></SelectTrigger>
                    <SelectContent>{profiles.map(p => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</SelectContent>
                  </Select>
                  {selectedProfileId && <Button variant="outline" size="icon" className="h-11 w-11 shrink-0" onClick={() => { const p = profiles.find(x => x.id === selectedProfileId); if(p) { setEditingProfileId(p.id); setNewProfileName(p.name); setNewProfileReq(p.requirements); setIsAddingProfile(true); }}}><Edit3 className="w-4 h-4" /></Button>}
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-200 bg-white shadow-sm">
            <CardHeader className="border-b border-slate-100 pb-3 flex items-center gap-2"><FileText className="w-5 h-5 text-blue-600" /><CardTitle className="text-base font-bold text-slate-800">简历原文内容</CardTitle></CardHeader>
            <CardContent className="pt-6 space-y-4">
              <Textarea placeholder="在此直接粘贴简历内容..." className="h-[440px] bg-white resize-none text-sm border-slate-200 focus:ring-2 focus:ring-blue-500/20 custom-scrollbar" value={resume} onChange={(e) => setResume(e.target.value)} />
              <Button className="w-full bg-slate-900 hover:bg-slate-800 h-12 text-lg font-black shadow-lg transition-transform active:scale-[0.98]" onClick={handleManualSubmit} disabled={loading || !selectedProfileId}>
                {loading ? <><Loader2 className="mr-2 animate-spin" />正在扫描解析...</> : <><Send className="mr-2 w-5 h-5" />开启 AI 深度扫描</>}
              </Button>
            </CardContent>
          </Card>
        </div>
                ...
                <Card key={job.id} className={`group border transition-all duration-300 relative ${isConfirming ? 'border-red-500 scale-[0.98] shadow-lg' : 'border-slate-200 bg-white hover:border-blue-200 hover:shadow-md'}`}>
                  ...
                  <CardContent className={`pt-6 ${isConfirming ? 'opacity-30 grayscale' : ''}`}>
                    <div className="flex justify-between items-start mb-6 pr-12">
                      <div className="space-y-2">
                        <div className="flex items-center gap-3">
                          ...
                          {/* ⭐ 核心修正：显式的匹配状态标签 */}
                          <div className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest border ${isMatch ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {/* ⭐ 匹配结论说明区 */}
                      <div className={`p-5 rounded-2xl border transition-all ${isMatch ? 'bg-green-50/50 border-green-100 text-green-900' : 'bg-red-50/50 border-red-200 text-red-900'}`}>
                        <div className="flex items-center gap-2 mb-3">
                          {isMatch ? <CheckCircle className="w-4 h-4 text-green-600" /> : <ShieldAlert className="w-4 h-4 text-red-600" />}
                          <p className="text-sm font-black tracking-tight">{isMatch ? '建议安排面试' : '暂不匹配该岗位'}</p>
                        </div>
                        <p className="text-xs font-semibold leading-relaxed mb-4 italic opacity-80">{res.match_reason}</p>
                        
                        <div className="pt-4 border-t border-black/5">
                          <p className="text-[9px] font-black uppercase mb-3 opacity-50 tracking-widest">Core Highlights</p>
